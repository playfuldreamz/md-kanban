...
function registerRoutes(app, ctx) {
  ...
  app.put('/api/cards/:id', (req, res) => {
    try {
      ...
      // Done toggle — auto-move top-level cards to/from Done column
      if (done !== undefined && !found.parentCard) {
        ...
        if (newDone && !oldDone && !isDoneColumn(found.column)) {
          // Marked done in non-Done column → move to Done
          moveCardToColumn(boardRef.current, card, found.column, found.index, 'done');
        } else if (!newDone && oldDone && isDoneColumn(found.column)) {
          // Unmarked done in Done column → move out
          const todo = boardRef.current.columns.find(c => !isDoneColumn(c));
          moveCardToColumn(boardRef.current, card, found.column, found.index, todo ? todo.id : found.column.id);
        }
  ...
  app.put('/api/cards/:id/move', (req, res) => {
    try {
      ...
      if (isDoneColumn(targetColumn) && !isDoneColumn(sourceColumn)) {
        setDoneRecursive(card, true);
      } else if (!isDoneColumn(targetColumn) && isDoneColumn(sourceColumn)) {
        setDoneRecursive(card, false);
      }
...
/**
 * Move a card from its source column to a target column.
 * Finds the target by id/name matching or creates a Done column if needed.
 */
function moveCardToColumn(board, card, sourceColumn, sourceIndex, targetHint) {
  // Find target column — prefer exact id match, then name match, then create
  let target = board.columns.find(c =>
    c.id === targetHint || c.id.includes(targetHint) ||
    c.name.toLowerCase().includes(targetHint)
  );
