...
function registerRoutes(app, ctx) {
  ...
  app.put('/api/cards/:id', (req, res) => {
    try {
      ...
      const { title, description, done, children, warning, pinned } = req.body;
      ...
      if (warning !== undefined) card.warning = Boolean(warning);
      if (pinned !== undefined) card.pinned = Boolean(pinned);
      card._changed = true;
      writeBoard(board, file);
      readAndParse(file);
  ...
  app.put('/api/cards/:id/move', (req, res) => {
    try {
      ...
      // Preserve plugins reference so serializer sees warning cards etc.
      if (!board._plugins && boards.get(file)?._plugins) board._plugins = boards.get(file)._plugins;
      writeBoard(board, file);
      readAndParse(file);
      res.json({ ok: true });
    } catch (err) {
  ...
  const { renderExportHtml } = require('./export-html');
  ...
  app.get('/api/board/export', (req, res) => {
    try {
      ...
      if (format === 'json') {
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="board-' + today + '.json"');
        return res.json(board);
      }

      if (format === 'csv') {
        const rows = [];
        rows.push(['column', 'parent_title', 'title', 'description', 'done', 'due_date', 'assignees', 'tags', 'created_at'].join(','));
        function flattenCards(cards, colName, parentTitle) {
          for (const card of cards) {
            const assignees = (card.assignees || []).join(',');
            const tagMatch = card.description ? card.description.match(/#([a-zA-Z0-9_-]+)/g) : null;
            const tags = tagMatch ? [...new Set(tagMatch.map(m => m.slice(1).toLowerCase()))].join(' ') : '';
            const desc = (card.description || '').replace(/"/g, '""');
            const ttl = (card.title || '').replace(/"/g, '""');
            const cols = [colName, parentTitle || '', ttl, desc, card.done ? 'true' : 'false', card.dueDate || '', assignees, tags, card.createdAt || ''];
            rows.push(cols.map(c => { const s = String(c); return (s.includes(',') || s.includes('"') || s.includes(String.fromCharCode(10))) ? '"' + s + '"' : s; }).join(','));
            if (card.children) flattenCards(card.children, colName, card.title);
          }
        }
        for (const col of board.columns) {
          flattenCards(col.cards, col.name, '');
        }
        res.setHeader('Content-Type', 'text/csv; charset=utf-8');
        res.setHeader('Content-Disposition', 'attachment; filename="board-' + today + '.csv"');
        return res.send(rows.join(String.fromCharCode(10)));
      }

      if (format === 'html') {
        const html = renderExportHtml(board);
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        res.setHeader('Content-Disposition', 'attachment; filename="board-' + today + '.html"');
