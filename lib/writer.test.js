...
test('serializeBoard: preamble round-trip — preserves format guide', () => {
...
`;
  const board = parseMarkdown(md);
  const serialized = serializeBoard(board);
  assert(serialized.includes('<!-- Format guide -->'), 'Should preserve preamble in output');
  assert(serialized.includes('# TODO'), 'Should include title after preamble');
  const reparsed = parseMarkdown(serialized);
  assert(reparsed.preamble.includes('<!-- Format guide -->'), 'Should survive round-trip');
  assertEqual(reparsed.title, 'TODO');
  assertEqual(reparsed.columns.length, 1);
  assertEqual(reparsed.columns[0].cards.length, 1);
});

test('serializeBoard: preamble — no preamble in board emits clean output', () => {
  const board = {
    title: 'Project',
    preamble: '',
    columns: [{ id: 'tasks', name: '📋 Tasks', emoji: '📋', cards: [] }],
  };
  const result = serializeBoard(board);
  assert(!result.includes('<!--'), 'Should not inject preamble when none exists');
  assert(result.startsWith('# Project'));
});

// ─── Sub-task serialization tests ───

test('serializeBoard: round-trip with sub-tasks', () => {
  const md = `## Column
- [ ] **Parent** — Desc
  - [x] Child 1
  - [ ] Child 2
...
`;
  ...
  assertEqual(reparsed.columns.length, 1);
  const parent = reparsed.columns[0].cards[0];
  assertEqual(parent.title, 'Parent');
  assert(parent.children, 'Should preserve children');
...
test('serializeBoard: sub-tasks — minimal diff on parent change only', () => {
  const md = `## Column
- [ ] **Parent** — Original
  - [ ] Child stays
  - [x] Child done
`;
  ...
  parent.title = 'Parent Changed';
  parent._changed = true;
  // Children unchanged

  const serialized = serializeBoard(board);
  assert(serialized.includes('Parent Changed'), 'Should include new title');
  assert(serialized.includes('Child stays'), 'Should preserve unchanged child');
  assert(serialized.includes('Child done'), 'Should preserve done child');
  assert(serialized.includes('- [x] Child done'), 'Should keep done checkbox');
});

test('serializeBoard: sub-tasks — child change rewrites child only', () => {
  const md = `## Column
- [ ] **Parent** — Desc
  - [ ] Child 1
  - [ ] Child 2
`;
  ...
  parent.children[0]._changed = true;

  const serialized = serializeBoard(board);
  assert(serialized.includes('- [ ] **Parent** — Desc'), 'Parent line unchanged');
  // Child 0 was changed → canonical format with [x] and **bold**
  assert(serialized.includes('  - [x] **Child 1**'), 'Child 1 should now be done');
  // Child 1 not changed → rawLine preserved verbatim
  assert(serialized.includes('  - [ ] Child 2'), 'Child 2 preserved verbatim');
});

test('serializeBoard: createdAt survives round-trip', () => {
  const md = `## Col
- [ ] **Task** — Desc <!-- created:2026-07-28 -->
`;
  const board = parseMarkdown(md);
  assertEqual(board.columns[0].cards[0].createdAt, '2026-07-28');
  const serialized = serializeBoard(board);
  assert(serialized.includes('<!-- created:2026-07-28 -->'), 'createdAt preserved in output');
  const reparsed = parseMarkdown(serialized);
  assertEqual(reparsed.columns[0].cards[0].createdAt, '2026-07-28');
});

test('serializeBoard: changed card appends createdAt', () => {
  const card = {
    id: 'abc', done: true, title: 'Done task', description: 'Finished',
    rawLine: '- [ ] **Done task** — Finished <!-- created:2026-06-01 -->',
    createdAt: '2026-06-01',
    _changed: true,
  };
  const result = serializeCard(card);
  assert(result.includes('- [x]'), 'Should be marked done');
  assert(result.includes('<!-- created:2026-06-01 -->'), 'Should preserve createdAt');
});
...
const dueDatesPlugin = require('./builtin/due-dates');

test('due-dates plugin: due:YYYY-MM-DD syntax', () => {
  const card = { description: 'Fix login — due:2026-08-15' };
  dueDatesPlugin.parseCard(card);
  assertEqual(card.dueDate, '2026-08-15');
});

test('due-dates plugin: 📅 YYYY-MM-DD syntax', () => {
  const card = { description: 'Fix login — 📅 2026-08-15' };
  dueDatesPlugin.parseCard(card);
  assertEqual(card.dueDate, '2026-08-15');
});

test('due-dates plugin: 📅 without space', () => {
  const card = { description: 'Fix login — 📅2026-08-15' };
  dueDatesPlugin.parseCard(card);
  assertEqual(card.dueDate, '2026-08-15');
});

test('due-dates plugin: no due date', () => {
  const card = { description: 'Fix login — no date here' };
  dueDatesPlugin.parseCard(card);
  assertEqual(card.dueDate, undefined);
});
